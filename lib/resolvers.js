...
module.exports = internals.resolvers = {
  Query: {
    ...
    image: async (root = {}, { id }, request) => {
      ...
      const [err, image] = await Intercept(CloudApi('fetch', { path: `/images/${id}` }, request));
  ...
  Mutation: {
    ...
    startMachineFromSnapshot: async (root, { id, snapshot: name }, request) => {
      const machine = await CloudApi('getMachine', { id }, request);
...
internals.resolvers = Object.assign(internals.resolvers, {
  ...
  Machine: {
    ...
    networks: (root, args, request) => {
      const { networks } = root;
      return Promise.all(
        networks.map(id => internals.resolvers.Query.network(root, { id }, request))
      );
    },
  ...
  FirewallRule: {
    machines: async (root, args, request) => {
      const { id } = root;
      return await CloudApi(root, 'listFirewallRuleMachines', { id }, request);
    }
  },
  Snapshot: {
    state: ({ state }) => (state ? state.toUpperCase() : state)
...
internals.toNameValues = obj => {
  ...
  return Object.keys(obj).map(name => {
    return {
      id: Hasha(JSON.stringify({ name: name, value: obj[name] })),
      name,
      value: obj[name]
    };
  });
