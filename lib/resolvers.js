...
module.exports = internals.resolvers = {
  Query: {
    rndName: async (root, args, request) => {
      const rndNameIndex = Math.floor(Math.random() * Names.length);
      const rndAdjIndex = Math.floor(Math.random() * Adjectives.length);
      const name = Names[rndNameIndex];
      const adjective = Adjectives[rndAdjIndex];
      ...
      const machinesList = await CloudApi('/machines', { query: { name: str } }, request);
    ...
    rndImageName: async (root, args, request) => {
      ...
      if (Sentiment(str.split(/-/).join(' ')).score < 0) {
        return internals.resolvers.Query.rndImageName(root, args, request);
      }

      const imagesList = await CloudApi('/images', { query: { name: str } }, request);
  ...
  Mutation: {
    ...
    createMachine: async (root, { name, image, networks, affinity = [], metadata, tags, firewall_enabled, ...args }, request) => {
      ...
      const payload = {
        ...
        ...internals.fromNameValues(tags, 'tag.'),
        ...internals.fromNameValues(metadata, 'metadata.'),
        firewall_enabled
      };
