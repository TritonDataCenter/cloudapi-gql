...
module.exports = {
  ...
  Machine: {
    ...
    image: (root, args, request) => {
      return Handlers.image(request.plugins.cloudapi.fetch, { id: root.image });
    },
    ...
    networks: (root, args, request) => {
      ...
      return Promise.all(
        networks.map((id) => {
          return Handlers.network(request.plugins.cloudapi.fetch, { id });
        })
      );
    },
  ...
  Network: {
    machines: async ({ fabric, id }, args, request) => {
      ...
      const machines = await Handlers.machines(request.plugins.cloudapi.fetch, request.payload);
